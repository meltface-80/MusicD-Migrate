...
const EDITION_WORDS = [
  ...
  // Added on the owner's decision after two real reports, each one the same
  // performances in a different pressing — the same family as "deluxe",
  // "expanded" and "anniversary edition" above:
  //   "(Bonus Edition)"          Coming On Strong
  //   "(Collector's Edition)"    Bummed
  //   "(International Version)"  Death to False Metal
  //   "(U.S. Version)"           Babylon        — canon writes it "u s version"
  //   "(2009 Re-Mastered)"       Emotional Rescue
  // What was asked for and deliberately NOT added: "extended version",
  // "(Remixes)", "(DJ Mix)", "(Acoustic)", "Vol. 2" and "(EP)". The first
  // four are different RECORDINGS, "Vol. 2" is a different record outright,
  // and an artist can have both an EP and an album of one name.
  "bonus edition", "collectors edition", "international version",
  "u s version",
];

/**
 * "(Re-Mastered)", "(2009 Re-Mastered)", "(Remastering)" — a remaster whose
 * spelling the word list does not carry.
 *
 * ANCHORED, and that is the point: `EDITION_WORDS` is matched as a substring,
 * so putting "re master" in the list also strips "(Pre-Master)", which is a
 * studio stage and not an edition. This form cannot.
 */
const REMASTER = /^(?:\d{4} )?re ?master(?:ed|ing)?$/;
