 ...
 *
 * The endpoint maximum on both services for a library write, which is what
 * makes writing as we go FREE: a flush of fifty makes exactly the request
 * that batch would have made in one write at the end. The clients chunk at
 * the same number.
 *
 * Writing at the end was the bug. A run that was cancelled, tripped a circuit
 * breaker, hit Android's six-hour foreground cap or simply had its process
 * killed wrote NOTHING, however many albums it had matched — and into Spotify
 * a ten thousand album library takes hours against a rate-limited search, so
 * any interruption in those hours saved nothing at all. Into Qobuz the phase
 * finished and the write happened, which is why only one direction looked
 * broken.
 */
...
class Migration {
  ...
  async run() {
    try {
      ...
      this.flush();
      // KEEP WHAT WAS MATCHED. A run that stops early — cancelled, a circuit
      // breaker, a dead sign-in, Android's foreground cap — has already found
      // real matches, and stranding them is what made a long run into Spotify
  ...
  async migrateSavedTracks(existing) {
    ...
    const w = this.writer("favourite tracks", (ids) => this.target.saveTracks(ids));
  ...
  async migrateSavedAlbums(existing) {
    ...
    const w = this.writer("favourite albums", (ids) => this.target.saveAlbums(ids));
  ...
  async migrateArtists(existing) {
    ...
    const w = this.writer("artists", (ids) => this.target.followArtists(ids));
    await this.eachWithConcurrency(source, async (a, i) => {
      this.checkCancelled();
   ...
   *
   * See WRITE_BATCH for why this is not an optimisation but a correctness
   * fix, and why it costs no extra requests.
   */
  writer(what, fn) {
    const self = this;
    const w = {
      ...
      async add(id) {
        w.ids.push(id);
        if (w.ids.length >= WRITE_BATCH) {
          // Spliced out synchronously, BEFORE any await, so two concurrent
          // workers cannot both carry off the same batch.
          await self.write(what, w.ids.splice(0, WRITE_BATCH), fn, true);
        }
      },
      async flush(quiet) {
        while (w.ids.length) {
          await self.write(what, w.ids.splice(0, WRITE_BATCH), fn, quiet);
        }
      },
    };
   ...
   *
   * Called from run()'s catch as well as its happy path: the whole point is
   * that a run which stops early keeps what it matched. Each writer is tried
   * even if an earlier one throws, because a failed album write is no reason
   * to strand the artists too.
   */
  async flushWrites() {
  ...
  async write(what, ids, fn, quiet) {
    if (this.options.dryRun) return;
    if (!ids.length) return;
    if (!quiet) this.report({ phase: "writing", label: `Saving ${ids.length} ${what}` });
    try {
      await fn(ids);
...
module.exports = { Migration, Cancelled, WRITE_BATCH, trackKey, albumKey, trackLabel,
