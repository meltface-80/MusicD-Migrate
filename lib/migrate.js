...
class Migration {
  ...
  async migratePlaylists() {
    ...
    const wanted = Array.isArray(this.options.playlists)
      ? all.filter((p) => this.options.playlists.map(String).includes(String(p.id)))
      : all.filter((p) => this.options.includeOthersPlaylists ||
                          !p.ownerId || p.ownerId === String(this.source.userId || ""));
  ...
  async resolveAlbum(a) {
    ...
    // The barcode is the album equivalent of an ISRC and is worth a request to
    // get: Spotify's search results carry it, Qobuz's usually do, and when
    ...
    const r = matchAlbum(cands, want, { strict: this.options.strict });
