 ...
 *
 * Worth writing down, because it is not guessable and it cost a release.
 *
 * Spotify has frozen new app registrations, so for most people the only
 * available Client ID is one of the well-known ones the open-source Spotify
 * ecosystem shares (librespot, ncspot, Spotty, SpotOn). Those registrations
 * whitelist exactly ONE loopback path — `/login` — on any 127.0.0.1 port
 * (RFC 8252). Send anything else and Spotify answers, before the user can even
 * sign in:
 *
 *     redirect_uri: Not matching configuration
 *
 * which is what `/api/spotify/callback` got. `/login` is what SpotOn
 * registers too (Plugins/SpotOn/API/PKCE.pm), and it is where that path came
 * from.
 *
 * Someone using their OWN Client ID registers whichever address this app shows
 * them, which is now this one — no worse for them, and it means a single path
 * works for everybody. The old path is still SERVED, so a redirect URI
 * registered before this change keeps working.
 */
const CALLBACK_PATH = "/login";

/** Still served, never advertised: for anyone who registered it against an app
 *  of their own before /login became the default. */
const LEGACY_CALLBACK_PATH = "/api/spotify/callback";

/**
 * The Client ID used when nobody has saved one.
 *
 * Spotify's registrations are frozen, so for the person this was built for
 * there is no such thing as "get your own" — asking for one that cannot be
 * obtained just makes the app unusable. So it is baked in, at the owner's
 * explicit request, with all of this stated in the open:
 *
 * - **It is not registered to MusicD Migrate.** It is one of the well-known
 *   ids the open-source Spotify world shares, and it works here for exactly
 *   one reason: `/login` is whitelisted on it (see CALLBACK_PATH).
 * - **The consent screen names that application, not this one**, and requests
 *   made with it count against its quota. If it is ever rate-limited or
 *   withdrawn, the Spotify half stops working and nothing here can fix that.
 * - **It is a fallback, never an override.** A saved id always wins, so
 *   anybody who does have a registration of their own pastes it in and never
 *   touches this.
 * - **It is not secret.** PKCE means there is no client secret, and a client
 *   id travels in the query string of the authorise URL in plain sight every
 *   time anyone signs in to anything.
 *
 * Kept in step with Pkce.DEFAULT_CLIENT_ID by hand; ContractTest fails if the
 * two ever disagree, because a mismatch would mean the phone and the
 * container sign in as two different applications.
 */
const DEFAULT_CLIENT_ID = "d420a117a32841c2b3474932e49fb54b";
 ...
 * @param {string} [path]  defaults to CALLBACK_PATH
 */
function callbackUrlFrom(req, path) {
  const h = (req && req.headers) || {};
  const p = path || CALLBACK_PATH;
...
module.exports = {
  AUTHORIZE_URL, TOKEN_URL, SCOPES, CALLBACK_PATH, LEGACY_CALLBACK_PATH,
  DEFAULT_CLIENT_ID,
