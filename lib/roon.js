...
const { canon, primaryArtist, artistNames } = require("./canon");
...
class RoonClient {
  ...
  async savedAlbums() {
    ...
    return rows.map((r) => ({
      ...
      title: r.title,
      // Split, not one glued string: Roon writes several album artists as
      // "Carla Bley/Steve Swallow/Andy Sheppard", and the engine searches on
      // artists[0]. See artistNames.
      artists: artistNames(r.artist),
