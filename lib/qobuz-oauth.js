 ...
 *
 * PORTED VERBATIM from meltface-80/MusicD-Remote (lib/qobuz-oauth.js), same
 * author, same licence. Only this comment differs: the original explains
 * itself in terms of the streaming waveform it was written for, and that
 * feature does not exist here. What it does is identical.
 *
 * WHY THE REDIRECT FLOW RATHER THAN A PASSWORD. Qobuz publishes a flow for
 * third-party apps: the user signs in on qobuz.com, Qobuz sends their browser
 * back here with a one-time code, and that code trades for a login token.
 * Nothing is typed into this app and no password passes through it — which
 * matters more here than it did there, because this app is the one that writes
 * to somebody's library.
 *
 * A username-and-password login also exists in lib/qobuz.js, because the
 * redirect cannot always get back (a headless server, a phone on a different
 * network). It is the fallback, not the default.
 *
 * THE REDIRECT URL IS DERIVED FROM THE REQUEST, not configured. Whatever
 * address the user reached the page on — 192.168.1.50:3380, a hostname, a
 * reverse proxy — is the address Qobuz is told to send them back to. That is
 * what makes this work inside Docker, where the container has no idea what
 * address it is reached on, and from a phone on the same network.
 *
 * Unlike Spotify, Qobuz accepts ANY redirect_url without pre-registration,
 * which is why this side needs no dashboard visit and no client id from the
 * user.
 *
