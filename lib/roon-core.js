...
class RoonCore {
   ...
   * means.
   *
   * This used to be `if (this._running) return`, and NOTHING cleared
   * `_running` but stop(), which only the server's own shutdown calls. So the
   * first attempt to be interrupted left the app unable to try again for the
   * rest of its life: every later press of "Find my Roon Core" did literally
   * nothing, silently. Reported as "fails to connect to Roon even after
   * enabling again in Roon extensions", and it is the reason that state was
   * unrecoverable rather than merely slow.
   *
   * A LIVE session is still left alone, and that part matters: pairing waits
   * for the user to click Enable in Roon and the approval comes back on that
   * very socket, so a double tap must not pull it out from under them.
   * Anything else — no session, or a closed one — is reconnected.
   */
  start() {
    if (this._running) {
      if (this._session && this._session.isOpen) return;
      const dead = this._socket;
      this._socket = null;
      if (dead) { try { dead.close("retrying"); } catch (e) { void e; } }
    }
    this._running = true;
    this._backoff = BACKOFF_START_MS;
