...
class Qobuz {
  ...
  async playlists() {
    ...
    return this.pageAll("playlist/getUserPlaylists", {},
      (r) => r && r.playlists && r.playlists.items, 500, (p) => p && ({
        id: String(p.id),
        name: p.name || "",
  ...
  async playlistTracks(playlistId) {
    return this.pageAll("playlist/get",
      { playlist_id: playlistId, extra: "tracks" },
      (r) => r && r.tracks && r.tracks.items, 500, toTrack);
  }

  async savedTracks() {
    return this.pageAll("favorite/getUserFavorites", { type: "tracks" },
      (r) => r && r.tracks && r.tracks.items, 500, toTrack);
  }

  async savedAlbums() {
    return this.pageAll("favorite/getUserFavorites", { type: "albums" },
      (r) => r && r.albums && r.albums.items, 500, toAlbum);
  }

  async followedArtists() {
    return this.pageAll("favorite/getUserFavorites", { type: "artists" },
      (r) => r && r.artists && r.artists.items, 500,
      (a) => a && ({ id: String(a.id), name: a.name || "" }));
  }

  /** Offset paging, with the section picked out by the caller because Qobuz
   *  names it differently on every endpoint. */
  async pageAll(endpoint, params, pick, limit, map) {
    const out = [];
    let offset = 0;
    const per = limit || 500;
    for (;;) {
      const r = await this.request(endpoint,
        Object.assign({}, params, { limit: per, offset }));
      const items = pick(r);
      if (!Array.isArray(items) || !items.length) break;
      // Mapped here, not by the caller, so one page of raw JSON is live at a
      // time rather than the whole library's. Qobuz's favourites are leaner
      // than Spotify's albums, but 6,767 of them is still 6,767 parsed
      // objects held for no reason. See lib/spotify.js.
